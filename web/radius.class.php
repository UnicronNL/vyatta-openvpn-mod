...
class Radius
{
    ...
    public function Radius($ip_radius_server = '127.0.0.1', $shared_secret = '', $radius_suffix = '', $udp_timeout = 5, $authentication_port = 1812, $accounting_port = 1813)
    {
        ...
        $this->SetNasPort();
        $this->SetServiceType();
        $this->SetFramedProtocol();
        $this->SetNASPortType();
        $this->SetNASIdentifier();
    ...
    function SetNASIdentifier($nas_identifier= '')
    {
        $this->_nas_identifier = $nas_identifier;
        $this->SetAttribute(32, $this->_nas_identifier);
    }

    function SetServiceType($service_type = 0)
    {
        $this->_service_type = intval($service_type);
        $this->SetAttribute(6, $this->_service_type);
    }

    function SetFramedProtocol($framed_protocol = 0)
    {
        $this->_framed_protocol = intval($framed_protocol);
        $this->SetAttribute(7, $this->_framed_protocol);
    }

    function SetNASPortType($nas_port_type = 0)
    {
        $this->_nas_port_type = intval($nas_port_type);
        $this->SetAttribute(61, $this->_nas_port_type);
    }
